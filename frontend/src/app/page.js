...
const MainPage = () => {
  ...
  const checkTransactionStatus = async (transactionId) => {
    try {
      const response = await fetch(`http://3.138.117.109/v1/transacciones/${transactionId}/estado`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://3.138.117.109/v1/procesamiento-transaccion/procesar", {
        method: "POST",
